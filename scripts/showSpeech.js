...
listOfPages = [["speech_submission", "Speech Submission"], ["explanation", "Congress Explanation"], ["rules", "Rules"], ["principles_and_theories", "Principles and Theories"], ["kritiks", "Types of Kritiks"], ["conlaw", "Constiutional Law"], ["domestic", "Domestic Issues"], ["foreign", "Foreign Policy"], ["econ", "Economics"], ["philosophical", "Philosophical"], ["counterplans", "Counterplans"], ["tutorials", "Tutorials"], ["search", "Search"]];
...
for (i = 0; i < listOfPages.length; i++) {
    ...
    $(".trigger").append("<a href='" + pageLink[0] + ".html' class='page-link'>" + pageLink[1] + "</a>");
};
...
var thisSpeechID = $(".speechID").attr("id");
listOfSpeeches.child(thisSpeechID).on("value", function(snapshot) {
    speech = snapshot.val();
    $(".page-content").append("<div class='wrapper'><div class='post'><header class='post-header'><h1 class='post-title'>" + speech.title + "</h1><p class='post-meta'>" + speech.dateFormatted + " * " + speech.author + "</p></header><article class='post-content'>" + speech.text + "</article></div></div>");
    $(".speechID").remove();
});
